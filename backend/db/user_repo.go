...
func GetMututalFollowers(user1, user2 string) ([]string, error) {
	ctx , cancel := context.WithTimeout(context.Background(), 5 * time.Second)
	defer cancel()
	...
	result, err := session.ExecuteRead(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
		...
		query := `
				 MATCH (f:User)-[:FOLLOWS]->(u1:User {user_id: $user1})
				 MATCH (f)-[:FOLLOWS]->(u2: User {user_id: $user2})
				 RETURN collect(f.user_id) AS mutuals
				 `
		...
		if res.Next(ctx) {
			...
			if raw != nil {
				for _, v := range raw.([]any) {
					if v != nil{
						mutuals = append(mutuals, v.(string))
					}
				}
			}
...
func GetUserMetrics(userID string) (map[string]int64, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5 * time.Second)
	defer cancel()
	...
	result, err := session.ExecuteRead(ctx, func(tx neo4j.ManagedTransaction) (any, error) {
		...
		query := `
				 ...
				 OPTIONAL MATCH (u)-[:FOLLOWS]->(g:User)
				 RETURN followers, count(g) AS following
			`
		...
		if res.Next(ctx) {
			...
			return map[string]int64{
				"followers": record.Values[0].(int64),
				"following": record.Values[1].(int64),
			}, nil
		}
		return nil, ErrUserNotFound
	})
